-- Migration 0043: player_watch（注目選手）を user 単位のデータへ変更（段階2）
--
...
--
-- 番号について: 0040は本来このマイグレーションの番号だったが、招待制ログイン機能を
-- minor（既存挙動を変えない追加）/major（挙動が変わる側）の2PRへ分割した際、
-- player_watchのuser単位化はmajor側に含めることにしたため、minor側（#24）では
-- 0040を欠番のまま残し0041/0042を採番した。番号の連番を無理に詰めるより
-- 欠番を許容する方が、既にマージ済みのminor側マイグレーション番号を変更せずに
-- 済むため、この番号（0043）は意図的な欠番の後を継いでいる。
--
