...
| `GITHUB_OWNER` | `packages/scraping/src/gateway/implement/githubIssueGateway.ts` | GitHubオーナー名。未設定時デフォルト `taichi6930` | Issue作成先リポジトリのオーナー |
| `GITHUB_REPO` | `packages/scraping/src/gateway/implement/githubIssueGateway.ts` | リポジトリ名。未設定時デフォルト `race-schedule` | Issue作成先リポジトリ名 |
| `GITHUB_TOKEN` | `packages/scraping/src/utility/githubMasterIssueNotifier.ts`, `packages/api/src/utility/dataFreshnessNotifier.ts` | GitHub APIトークン文字列 | GitHub Issue通知機能の認証（scraping: 未対応ステージ検出、api: データ鮮度チェック[CICD-121]）。トークン種別ごとの使い分け・発行元は[`packages/README.md`](../packages/README.md#githubトークンの使い分け種類権限スコープ使用箇所)参照 |
