...
- **対象**: `packages/core`・`packages/admin`・`packages/batch`・`packages/api`。
  ...
  `packages/core/src/domain/master/`（レース場・グレード等のコード対応表）も同様の理由で
  対象外（2026-08-22）: 分岐を持たない素のリテラルデータの羅列で、ミューテーションを
...
### PR差分スコープのミューテーションテスト（`mutation-diff-report.yml`, 情報提供のみ）

週次実行とは別に、PRで変更されたsrcファイルだけを対象にその場でStrykerを実行し、
結果をPRコメントとして表示する仕組みがある。**非ブロッキング**（マージの可否には
一切影響しない）。
...
- **ブロッキング化を見送っている理由**: (1) command runnerが1ミュータントごとに
  ...
  対象パッケージのテストスイート規模に比例するため、大きいパッケージほど
  1ファイルの差分でも時間がかかる）。(2) equivalent mutant（原理的に殺せない
  ミュータント）やリファクタによるミュータント構成の変化でスコアが変動しやすく、
  ブロッキングゲートはこの手の誤検知に弱い。(3) 週次実行を数回試した程度では
  ノイズの実態を判断する材料が不足している。ブロッキング化するかどうかは、
  この情報提供運用をしばらく回してから改めて判断する（ユーザー確認済み、
  2026-08-22）。
- **見つかったギャップの扱い**: PRコメント上でsurvivedミュータントが見つかっても
  対応は必須ではなく、PR作成者の判断に委ねる（このPRで直しても、後回しにしても
  よい）。継続的な改善は引き続き `docs/tasks/test-quality-audit.md` のバックログで
  扱う。
